...
public class ProductInfo extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            {
                out.println("<form class=frm1 method='POST' action='ProductInfo?id=" + pid + "'>" +
            ...
            try {
                ...
                String sql= "SELECT * FROM Product JOIN Typeof ON Product.typeid=Typeof.id" +
                        "                          JOIN Manuf ON Product.manufid=Manuf.id" +
                        "                          JOIN Info ON Product.id=Info.pid" +
                        "                          JOIN Color ON Info.colorid=Color.id" +
                        "                          JOIN Category ON Info.catid=Category.id" +
                        "                   WHERE Product.id = " +  pid;
                rs = stat.executeQuery(sql);
                rs.first();
                
                out.println("<p><img src=\"" + rs.getString("PHOTO") + "\" width=200 height=200></p>" +
                            "<p>" + rs.getString("NAMEOF") +  "</p>" +
                            "<p>" + "$" + rs.getDouble("PRICE") + "</p>" +
                            "<p> -------------------- </p>" +
                            "<p>" + "Information about the product:" + "</p>" +
                            "<p>" + "For Type: " + rs.getString("TYPEOF") + "</p>" +
                            "<p>" + "Category: " + rs.getString("CATEGORY") + "</p>" +
                            "<p>" + "Manufacturer: " + rs.getString("MANUF") + "</p>");
                if(rs.getString("Color") != null)
                    out.println("<p>" + "Color: " + rs.getString("Color") + "</p>");
                if(rs.getString("SIZEOF") != null)
                {
                    if(rs.getInt("CATID")==1)
                        out.println("<p>" + "Capacity: " + rs.getString("SIZEOF") + "</p>");
                    else if(rs.getInt("CATID")==3 || rs.getInt("CATID")==4)
                        out.println("<p>" + "Length: " + rs.getString("SIZEOF") + "</p>");
                    else
                        out.println("<p>" + "Size: " + rs.getString("SIZEOF") + "</p>");
                }
                boolean inStock = false;
                if(rs.getInt("STOCK")>0)
                {
                    out.println("<p>" + "Stock: " + rs.getInt("STOCK") + "</p>");
                    inStock=true;
                }
                else
                    out.println("<p>" + "Stock: Out Of Stock!</p>");

                if(rs.getString("SPECK") != null)
                    out.println("<p>" + "Spec: " + rs.getString("SPECK") + "</p>");

                rs.close();
                stat.close();
                con.close();

                if(inStock && !session.getAttribute("UserId").equals(0))
                {
                    out.println("<form>" +
                    "<input type=\"button\" value=\"Add to Cart\" Onclick=\"window.location.href='http://localhost:8080/WebApplication2/Cart?id=" + pid + "'\" >" +
                    "</form>" );
                }
                else if(inStock) 
                {
                    out.println("<p><b> You need to sign in to buy this product! </b></p>");
                }
