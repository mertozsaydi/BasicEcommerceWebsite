    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            try {
                ...
                rs = stat.executeQuery("SELECT id, nameof, price, photo FROM Product");
                rs.first();
            
                do{
                    if(request.getParameter("keyword")!=null)
                    {
                        if(rs.getString("NAMEOF").toLowerCase().indexOf(request.getParameter("keyword").toLowerCase()) != -1)
                        {
                            out.println("<tr>" +
                            "<td align=\"center\"><img src=\"" + rs.getString("PHOTO") + "\" width=\"60\" height=\"60\"></td>" +
                            "<td><a href='ProductInfo?id=" + rs.getInt("ID") + "'>" + rs.getString("NAMEOF")  + "</a></td>" +
